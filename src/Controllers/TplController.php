...
class TplController extends Controller {
	...
	public function __construct() {
		$tplbase = config("apcu.tpl_base");
		$this->tplbase = base_path($tplbase); //TPL根目录
		$this->tpl_cache = config('apcu.tpl_cache');
	...
	private function getFileText($list, $type = 'css', $tplname = ""){
		if(empty($list)) return "";//dump($list);
		$str = "";
		$remark_l = "/************  ";
		$remark_r = "  ************/";
		if($type == 'css'){
			foreach ($list as $key=>$val){
				...
				$filepath = $this->tplbase.str_replace(".", "/", $val)."/tpl.css";
				if(is_file($filepath)){
					$tstr = $this->getFileTextIn($filepath, $class)."\n\n";
				}
				$filepath = $this->tplbase.str_replace(".", "/", $val)."/tpl.scss";
				if(is_file($filepath)){
			...
			foreach ($list as $key=>$val){
				...
				$class = str_replace("/", "_", $class);
				$filepath = $this->tplbase.str_replace(".", "/", $val)."/tpl.js";
