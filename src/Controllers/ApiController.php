...
use \Apcu\Controllers\VimController as Vim;
...
class ApiController
{
    ...
    public function setConfig(){
        list($this->retconfig, $this->vconfig, $vimconfig, $this->allfield) = $this->vim->get_data_config($this->tplclass);
        $vconfig = &$this->vconfig;
        $operwidth = 20; //单个操作宽度
    ...
    private function setHandles(&$handles){
        $tpl = $this->tpl;
        $ida = $this->tplclass->userinfo['menu_ida'];
        empty($md5s) && $md5s = [];
    ...
    private function getTreeListAll($tree, $data=[]){
        ...
        $twhere = $tree['where'];
        $where = $this->vim->get_where_real_list($twhere, $dbinfo);
    ...
    protected function setHandleConfig(&$handle, $vconfig, $edit_type, $field_key){
        ...
        if($vconfig['status']){
            ...
            if($type == 'tree'){
                $data = $this->data['_'];
                if(is_string($data)) EXITJSON(0, $data);
    ...
    private function getFromCheck($from){
        ...
        if(is_array($vthis) && count($vthis) > 1 && is_array($vlink) && count($vlink) > 1){
            ...
            if(!isset($allfield[$vthiskey])){
                EXITJSON(0, "{$def_conn}->{$def_table}字段{$vthiskey}不存在！");
            }
            $vtableinfo = $vtables[$vtable]['field'];
            if(!isset($vtableinfo[$vthisfield])){
                EXITJSON(0, "{$vconn}->{$vtable}字段{$vthisfield}不存在！");
            }
            ...
            if(!isset($btables[$btable])){
                EXITJSON(0, "{$bconn}->{$btable}不存在！");
            }
            $btableinfo = $btables[$btable]['field'];
            if(!isset($btableinfo[$vlinkkey])){
                EXITJSON(0, "{$bconn}->{$btable}字段{$vlinkkey}不存在！");
            }

            if(!isset($vtableinfo[$vlinkfield])){
                EXITJSON(0, "{$vconn}->{$vtable}字段{$vlinkfield}不存在！");
            }
...
//            dump($vlist);
    ...
    protected function listConfig($vimconfig, $allfield){
        ...
        if(count($vconfig_src) > 4){
            ...
            $menu_field = $this->tplclass->cache(function () use($_mid_, $user_id){
                $mf_info = $this->tplclass->db($GLOBALS['DOMAIN_CONFIG']['conn']."_menu_field", 'user')->where('menu_id', '=', $_mid_)->where('user_id', '=', $user_id)->first();
                if(empty($mf_info)){
                    return false;
                }
        ...
        if($is_tree){
            ...
            if($this->tplclass->isPost()){
                if($_POST['type'] == 'all'){
                    ...
                    !isset($_POST["value"]) && EXITJSON(0, "没有数据传递");
                    $value = $_POST["value"];
    ...
    protected function editConfig($type='edit'){
        ...
        if($this->tplclass->isPost()){
            ...
            if(empty($gedata) && empty($ext_data)) EXITJSON(0, "没有数据传递");
            if(!empty($gedata)) {
                $data = [];
            ...
            if(!empty($keywheres)){
                ...
                if(!empty($fst)){
                    ...
                    if(count($keyfieldnames) > 1){
                        EXITJSON(0, "字段组合 (".implode("、", $keyfieldnames).") 不能重复，在".$str."中");
                    }else{
                        EXITJSON(0, "字段 (".$keyfieldnames[0].") 不能重复，在".$str."中");
                    }
                }
            }
        }
        ...
        foreach ($handle as $key=>$val){
            ...
            if($val['type'] == 'password'){
                if($val['md5']){
                    ...
                    $field[$key] = $xcrypto->aesDecrypt($field[$key]);
                }elseif($val['des']){
                    // des解密
                    if(!isset($xcrypto)){
                        $xcrypto = import('XCrypto');
                    }
                    $field[$key] = $xcrypto->desDecrypt($field[$key]);
                }
            }
            if(empty($json)){
    ...
    protected function edit($type='edit'){
        ...
        $data = $this->data['_'];
        is_string($data) && EXITJSON(0, $data);
        $src = $this->data['src'];
        empty($src) && $src = [];
        $pkmd5s = $this->getPkMd5s($src);
        $retdata = [];
    ...
    protected function deletesConfig($type="all"){
        ...
        if(!$this->tplclass->isPost()) EXITJSON(0, "500 ERROR");
        $data = $_POST['data'];
        if(empty($data) && !is_array($data)) EXITJSON(0, "无数据传递！");
        $db = $this->tplclass->db();
        $wi = 0;
        $pkarr = [];
        $pkkv = [];
    ...
    protected function handleConfig(){
        ...
        $pks = $this->getPks();
        if(empty($pks) || !is_array($pks)) EXITJSON(0, "主键设置无效");
        $hinit = $this->vimconfig_handleinit;
        if(empty($hinit)) EXITJSON(0, "参数设置无效");
        ...
        if(empty($data)) EXITJSON(0, "数据设置出错");
    ...
    protected function upload(){
                ...
                foreach ($_FILES as $key=>$val) {
                    ...
                    if ($pos > 0) {
                        ...
                        if(!in_array($file_ext, $format)){
                            EXITJSON(0, "格式 {$file_ext} 不支持");
                        }
                    }else{
                        EXITJSON(0, "上传后缀名不能为空");
                    }
                }
            }
        }else{
        ...
        if(empty($fileinfo)) EXITJSON(0, "404 ERR");
    ...
    protected function selectTreeConfig(){
        ...
        empty($tree) && EXITJSON(0, "数据传递无效");
        (!isset($_POST['value'])) && EXITJSON(0, "数据传递无效");
        $value = $_POST['value'];
