...
return function($data){
	if(!class_exists('Scssc')) {
		...
		class Scssc
		{
			...
			public function css_compress($string){
				//压缩
				$string = str_replace("\r\n","",$string); //首先去掉换行
				$string = preg_replace("/(\s*\{\s*)/","{",$string);
				$string = preg_replace("/(\s*\}\s*)/","}",$string); //去掉反括号首位的空格和换行，和最后一个;
				$string = preg_replace("/(\s*\;\s*)/",";",$string);
				return $string;
			}

			//CSS压缩
			public function css_decompress($string){
				//解压
				$string = $this->css_compress($string); //为了效果更好，解压前，先压缩至最简状态
				$string = str_replace("{"," {\r\n    ",$string);
				$string = str_replace(";",";\r\n    ",$string);
				$string = str_replace("*/","*/\r\n",$string);
				$string = str_replace("    }","}\r\n\r\n",$string);
