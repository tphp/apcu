...
class ApiConf {
     ...
     */
    public function getHttpData($url, $para=null, $method=null, $header=NULL, $output_encoding = false, $iscurl = true) {
        $crf = SESSION_ID_MD5;
        if(strpos($url, "?") > 0){
            $url .= "&crf=".$crf;
        }else{
        ...
        if($this->obj->viewdata['is_login']){
            $method = $this->obj->tpl_type;
            $url .= "&menu_dir={$this->obj->tpl_path}&pro={$GLOBALS['DOMAIN_CONFIG']['key']}&is_login=true&type={$method}";
        }
