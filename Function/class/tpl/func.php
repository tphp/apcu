	...
	if(!class_exists('Tpl')) {
		...
		class Tpl extends ExtController{
			...
			function __construct($tpl = '', $roottpl = '') {
				...
				$this->config_tplbase = config("apcu.tpl_base");
				$this->tpl_type = "";
                $tpl === false && $tpl = '';
				if(!empty($tpl)){
					$tpl = str_replace("\\", "/", $tpl);
				...
				$this->apifun = new \Apcu\Controllers\ApiController($tpl, $this);
                $rtt = "";
				if(TPL::$is_root) {
                    $this->is_root = true;
                    TPL::$is_root = false;
			...
			private function getRetData(){
				...
				\Apcu\Controllers\PageController::$pages = $pages;
			...
			public function dbInfo($conn="", $table=-100, $field=""){
                $sql = new \Apcu\Controllers\SqlController();
                $data = $this->setDataInfo();
			...
			private function getMd5info($type = 'css'){
				...
				if($type == 'css') {
					foreach ($class as $key => $val) {
						...
						if (is_file($fcsspath . "{$type}") || is_file($fcsspath . "s{$type}")) $retarr[] = $val;
					}
				}else{
					foreach ($class as $key => $val) {
						if (is_file($this->tplbase . $val . "/tpl.{$type}")) $retarr[] = $val;
					}
				}
