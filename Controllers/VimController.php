 ...
 */
class VimController extends SqlController{
     ...
     * @return array
     */
	private function get_vim_file($tplclass){
     ...
     * @return array
     */
    private function get_allfield($config){
        if(in_array($config['type'], ['sql', 'sqlfind'])) {
            $c = $config['config'];
            empty($c['conn']) && $c['conn'] = config('database.default');
            return $this->dbInfo($c['conn'], $c['table']); //获取某个表的所有字段信息
        }
        return [];
    }
     ...
     * @return array
     */
    private function get_allfield_ext($table="", $conn=""){
        empty($conn) && $conn = config('database.default');
        return $this->dbInfo($conn, $table); //获取某个表的所有字段信息
    }
	 ...
	 * @param $vimfield
	 */
	private function get_new_config($config, $vimconfig, &$allfield){
        if(in_array($config['type'], ['sql', 'sqlfind'])) {
     ...
     * @param $fields
     */
	private function set_fields(&$fields, $config){
                        ...
                        if(!empty($fds) && !empty($fds[$fd]) && !empty($fds[$fv])){
                            ...
                            if(isset($from_isset[$tflag])){
                                ...
                                if(!empty($fw) && is_array($fw)){
                                    $fw = $this->get_where_real_list($fw, $fds);
     ...
     * @return array
     */
    public function get_where_real_list($where, $field=[]){
        $swhere = [];
	 ...
	 * @param $tpl
	 */
	public function get_data_config($tplclass){
		$config = $tplclass->getDataFile("vim");
        $allfield = $this->get_allfield($config);
		$vimconfig = $this->get_vim_file($tplclass);
		if(is_array($vimconfig['field'])) {
            foreach ($vimconfig['field'] as $key => $val) {
        ...
        $this->set_fields($vimconfig['field'], $config);
        $this->set_fields($vimconfig['handle'], $config);
		...
		list($fconfig, $vconfig, $isfixed) = $this->get_new_config($config, $vimconfig, $allfield);
        $retconfig = [];
		if($vimconfig['ispage']) {
